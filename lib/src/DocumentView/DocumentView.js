...
export class DocumentView extends PureComponent {
  ...
  setAnnotationDisplayAuthorMap = (authorMap) => {
    const tag = findNodeHandle(this._viewerRef);
    if (tag != null) {
      return DocumentViewManager.setAnnotationDisplayAuthorMap(tag, authorMap);
    }
    return Promise.resolve();
  };
  ...
  setColorPostProcessMode = (colorPostProcessMode) => {
    const tag = findNodeHandle(this._viewerRef);
    if (tag != null) {
      return DocumentViewManager.setColorPostProcessMode(
        tag,
